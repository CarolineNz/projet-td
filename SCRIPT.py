...
#Arguments : action, (complément action), (id capteur), variable 1, (variable 2), (date 1), (date 2)

if action == "display" :
    if sys.argv[2] == "humidex" :
        if sys.argv[3].isdigit() : #id capteur donné
            if len(sys.argv) == 6 : #les dates sont précisées
                humidex(int(sys.argv[3]), sys.argv[4], sys.argv[5])
            else :
                humidex(sys.argv[3])
        else :#pas d'id capteur
            if len(sys.argv) == 5 : #les dates sont précisées
                humidex_all(sys.argv[3], sys.argv[4])
            else :
                humidex_all()
        ...
        if sys.argv[2].isdigit(): #id capteur donné
            if len(sys.argv) == 6 : #les dates sont précisées
                display(int(sys.argv[2]), sys.argv[3], sys.argv[4], sys.argv[5])
            else :
                display(int(sys.argv[2]), sys.argv[3])
        else : #pas d'id capteur
            if len(sys.argv) == 5 : #les dates sont précisées
                display_all(sys.argv[2], sys.argv[3], sys.argv[4])
            else :
    ...
    if sys.argv[2].isdigit() : #id capteur donné
        if len(sys.argv) == 6 : #les dates sont précisées
            display_stat(int(sys.argv[2]),sys.argv[3], sys.argv[4], sys.argv[5])
        else :
            display_stat(int(sys.argv[2]),sys.argv[3])
    else :
        print("Erreur : veuillez préciser l'id du capteur (forme : displayStat [id] [variable]")


elif action == "corrélation" :
    if sys.argv[2].isdigit() : #id capteur donné
        if len(sys.argv) == 7 : #les dates sont précisées
            correlation(int(sys.argv[2]), sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6])
        else :
            correlation(int(sys.argv[2]), sys.argv[3], sys.argv[4])
    else :
        if len(sys.argv) == 6 : #les dates sont précisées
            correlation_all(sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5])
        else :
            correlation_all(sys.argv[2], sys.argv[3])




elif action == "analyse" :
    if sys.argv[2] == "moyenne" :
        analyse_moy_all()
    ...
    print("Veuillez renseigner les arguments dans l'ordre suivant :")
    print("action, (complément action), (id capteur), variable 1, (variable 2), (date 1), (date 2)")
    print("")
    print("Actions possibles :")
    print("display \ndisplay humidex \ndisplay capteurs \ndisplayStat (id de capteur obligatoire) \ncorrélation \nanalyse moyenne (pas d'autres arguments) \nanalyse position (pas d'autres arguments")